...
#    #ERRR
    def breakDegen( self):
        ...
        new_glob1RDM =  utils.rot1el( self.n4, self.tot_system.NOevecs )  
        new_NOeval, new_NOevecs = np.linalg.eigh(new_glob1RDM)
        
        print("evals", new_NOeval)
        glob_init_diag = utils.rot1el( init_glob1RDM, new_NOevecs )
        print("new_glob1RDM",new_glob1RDM)
        self.tot_system.glob1RDM  = np.copy(new_glob1RDM)
        self.tot_system.get_nat_orbs()
        ...
        if (np.allclose(np.sort(self.tot_system.NOevals)[::-1], self.tot_system.NOevals, 1e-12) == False):
            ...
            self.tot_system.NOevals = np.real(np.copy(self.tot_system.NOevals[idx]))
            self.tot_system.NOevecs = np.copy(self.tot_system.NOevecs[:, idx])
        
        print("self.tot_system.NOevals", self.tot_system.NOevals)
        print("self.tot_system.NOevecs", self.tot_system.NOevecs)

        #self.tot_system.get_new_mf1RDM(round(self.tot_system.Nele/2))
        #self.tot_system.V_site = 0
        #self.tot_system.corr_emb_calc(self.nproc) 
        #self.tot_system.V_site = 1
        #quit()
    #####################################################################
    def integrate( self, nproc ):
        ...
        #    print("FINISHED STEP:", self.step)
            ...
            if np.allclose(eve_no_diag, zero, 1e-12) == False:
