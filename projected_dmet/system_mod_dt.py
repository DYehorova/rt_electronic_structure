...
import numpy as np
import real_time_elec_structureGN.projected_dmet.fragment_mod_dt as fragment_mod
import sys
#Errr
#import fragment_mod
import os
 ...
 #       for i in range(Nfrag):
  #          self.frag_list.append( fragment_mod.fragment( impindx[i], Nsites, Nele ) )
    ...
    def get_glob1RDM( self ):
        ...
        #unpack necessary stuff (NOTE here we're assuming all fragments have same number of impurities)
    ...
    def corr_emb_calc( self, nproc ):
        ...
        if( not self.periodic ):
            ...
            if( nproc == 1 ):
                ...
                self.frag_list = frag_pool.starmap( self.static_corr_calc_wrapper, self.frag_list )
                frag_pool.close()
                frag_pool.join()
        else:
            print('ERROR: Do not currently have a periodic version of the code')
            exit()
    ...
    def get_frag_iddt_corr1RDM( self ):
        ...
        for frag in self.frag_list:
            frag.get_iddt_corr1RDM( self.h_site, self.V_site, self.hamtype, self.hubsite_indx )
