...
import numpy as np
import real_time_elec_structureGN.projected_dmet.system_mod_dt as system_mod
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            if( np.mod( step, self.Nprint ) == 0 ) and step > 0:
                ...
                print("time for output files", time.time()-save_time)

                sys.stdout.flush()
            if( np.mod( step, self.Nprint*10 ) == 0 ) and step > 0:
                print("saving restart system")
                pickle_time = time.time()
                file_system = open( 'restart_systemJ.dat', 'wb' )
                pickle.dump( self.tot_system, file_system )
                file_system.close()
                print("time to pickle", time.time()-pickle_time)
 ...
 #           print(self.tot_system.NOevecs)
  ...
  #          print("dt for rotmat", k1_list)
    ...
    def min_dif(self, array):
        ...
        for i in range(n-1):
            for j in range(i+1, n):
                if(abs(array[i]-array[j])) < diff and (abs(array[i]-array[j])) > self.dG :
    ...
    def one_rk_step( self, nproc ):
        ...
        frag_corr_time = time.time()
        self.tot_system.get_frag_corr12RDM()
        print("get corr12RDM", time.time()-frag_corr_time)
       #if self.substep == 3:
 #       print("############### GLOB CHECK #####################")
  ...
  #      print(np.allclose(projected_MF, self.tot_system.mf1RDM,rtol=0, atol=10e-10))
   ...
   #     print("Diagonalizing env of mf RDM with R after making new rdm")
    #    self.tot_system.eigvec_frag_MF_check()
        ...
        print("make derivative", time.time()-make_derivative)
        ...
        print("make xmat", time.time()-make_xmat)
        #ERR
        ...
        if( nproc == 1 ):
            ...
            frag_pool.close()
            #frag_pool.join()
        print("apply ham to ci coeff", time.time()-apply_ham)
    ...
    def print_data( self, current_time ):
        ...
        print("time corr dense", time.time()-corr_dense_time)
        #Print output data
