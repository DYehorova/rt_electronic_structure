...
class fragment():
    ...
    def get_Hemb( self, h_site, V_site, hamtype=0, hubsite_indx=None ):
        ...
        if( hamtype == 0 ):
            ...
            self.V_emb = V_site*np.einsum( 'ap,cp,pb,pd->abcd', utils.adjoint( rotmat_vsmall ), utils.adjoint( rotmat_vsmall ), rotmat_vsmall, rotmat_vsmall, optimize=True )
        ...
        if( hamtype == 0 ):
            ...
            core_int = V_site * np.einsum( 'ap,pb,p->ab', utils.adjoint( rotmat_vsmall ), rotmat_vsmall, np.einsum( 'pe,ep->p',rotmat_small[hubsite_indx,2*self.Nimp:], utils.adjoint( rotmat_small[hubsite_indx,2*self.Nimp:] ) ), optimize=True )
    ...
    def get_iddt_corr1RDM( self, h_site, V_site, hamtype=0, hubsite_indx=None ):
        ...
        if( hamtype == 0 ):
            ...
            IFmat += V_site*np.einsum( 'ap,pb,p->ab', utils.adjoint(rotmat_Hub), rotmat_Hub, tmp, optimize=True )
        ...
        if( hamtype == 0 ):
            ...
            #Hubbard hamiltonian
            tmp = np.einsum( 'pc,cd,dp->p', rotmat_Hub[:,actrange], self.corr1RDM, utils.adjoint(rotmat_Hub[:,actrange]), optimize=True )
            AFmat = 0.5 * V_site * np.einsum( 'ap,pb,p->ab', utils.adjoint(rotmat_Hub), rotmat_Hub, tmp, optimize=True )
        ...
        if( hamtype == 0 ):
            ...
            genFmat[ self.corerange, : ] = np.transpose( 2*( IFmat[:,self.corerange] + AFmat[:,self.corerange] ) )
            genFmat[ actrange, : ] =  np.transpose( np.dot( IFmat[:,actrange], self.corr1RDM ) )
            tmp = np.einsum( 'dp,pc,pe,bcde->pb', utils.adjoint( rotmat_Hub[:,actrange] ), rotmat_Hub[:,actrange], rotmat_Hub[:,actrange], self.corr2RDM, optimize=True )
    ...
    def get_Xmat( self, mf1RDM, ddt_mf1RDM ):
     ...
     #   #core-bath and core-virt
        for b in self.corerange:
            for a in np.concatenate( (self.bathrange, self.virtrange) ):
                if( a != b and np.abs( env1RDM_evals[a-self.Nimp] - env1RDM_evals[b-self.Nimp] ) > 1e-5 ):
     ...
     #   #Multiply difference in eigenalues and rotated time-derivative matrix
        ...
        print("xmat1 time:", time.time()-xmat1_time)
        self.Xmat_test2 = np.zeros( [self.Nsites, self.Nsites], dtype=complex )
        self.Xmat_test2 = np.triu(self.Xmat) + np.triu(self.Xmat,1).conjugate().transpose()
     #   ############ METHOD 2 ############
        xmat2_time = time.time()
        ...
        self.Xmat_test = np.triu(self.Xmat_test) + np.triu(self.Xmat_test,1).conjugate().transpose()
        print("xmat 2 time:", time.time()-xmat2_time)
        print("X1 and X1 hermit:", np.allclose(self.Xmat_test2, self.Xmat, rtol=0, atol=1e-12))
       # if np.allclose(self.Xmat_test2, self.Xmat, rtol=0, atol=1e-12) is False:
   #         print("differance", self.Xmat-self.Xmat_test2)
        print("X2 and X1 hermit:", np.allclose(self.Xmat_test, self.Xmat_test2, rtol=0, atol=1e-12))
      #  if np.allclose(self.Xmat_test, self.Xmat_test2, rtol=0, atol=1e-12) is False:
         #   print("differance", self.Xmat_test-self.Xmat_test2)
        print("X2 and X1 :", np.allclose(self.Xmat, self.Xmat_test, rtol=0, atol=1e-12))
       # if np.allclose(self.Xmat_test, self.Xmat, rtol=0, atol=1e-12) is False:
      #      print("differance", self.Xmat-self.Xmat_test)
     #   if np.allclose(self.Xmat_test, self.Xmat, rtol=0, atol=1e-5) is False:
    #        print("difference between x1 and x2 is greater then 10-5")
            #quit()
        self.Xmat = np.copy(self.Xmat_test2)
        ########## Method 3 ############
