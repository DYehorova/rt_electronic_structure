...
def get_ddt_mf1rdm_serial( system, Nocc ):
   ...
   # print("G", G)
    ...
    print( np.allclose(G, G.conjugate().transpose(), rtol=0.0, atol=1e-12 ), np.allclose(G_site, G_site.conjugate().transpose(), rtol=0.0, atol=1e-12 ) )
    #print(np.allclose(ddt_mf1RDM, chk, rtol=0.0, atol=1e-12 ) )
    ...
    print("#########################")
    print("check for derivative in NOs with Comm", np.allclose(chk, ddt_mf1RDM, 1e-12))
    print("#########################")
    print("check for derivative in NOs without Comm", np.allclose(chk, ddt_mf1RDM_test, 1e-12))
...
def calc_Gmat( system, iddt_glob1RDM ):
    ...
    for a in range(system.Nsites):
        for b in range(system.Nsites):
            if( a != b and np.abs( evals[a] - evals[b] ) > 1e-6 ):
                eval_dif[a,b] = 1.0 / ( evals[b] - evals[a] )

    print("eval_dif", eval_dif)
