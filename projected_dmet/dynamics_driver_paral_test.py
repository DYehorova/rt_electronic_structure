...
import numpy as np
import real_time_elec_structureGN.projected_dmet.system_mod_paral_test as system_mod
  ...
  #      print(np.allclose(projected_MF, self.tot_system.mf1RDM,rtol=0, atol=10e-10))
   ...
   #     print("Diagonalizing env of mf RDM with R after making new rdm")
    ...
    # print("ddt_mf1RDM", ddt_mf1RDM)
        ...
        change_rotmat_list_true = []
        for frag in self.tot_system.frag_list:
            change_rotmat_list_true.append( -1j * self.delt * np.dot( frag.rotmat, frag.Xmat ) )
...
#            frag_pool.join()
        ...
        for ifrag, frag in enumerate(self.tot_system.frag_list):
            ...
            change_CIcoeffs_list.append( applyham_wrapper( frag, self.delt, ddt_mf1RDM, self.tot_system.mf1RDM ) )
        print("time without paralelization:", "--- %s seconds ---" % (time.time()-no_paralel_start))
        change_rotmat_list_test = []
        for frag in self.tot_system.frag_list:
            change_rotmat_list_test.append( -1j * self.delt * np.dot( frag.rotmat, frag.Xmat ) )
        print(np.allclose(change_rotmat_list_test, change_rotmat_list_true, rtol=0, atol=1e-12))
        quit()
...
def applyham_wrapper( frag, delt, ddt_mf1RDM, mf1RDM ):
    ...
    frag.get_Xmat( mf1RDM, ddt_mf1RDM ) 
