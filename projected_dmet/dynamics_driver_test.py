...
import numpy as np
import real_time_elec_structureGN.projected_dmet.system_mod_copy as system_mod
...
#        quit()
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            self.curr_time = np.copy(current_time)
          ...
          #  if  abs(eval_mindif) < 1e-4:
           ...
           # else:
            #    self.delt = 0.001  
                self.time_counter = 0
         # if abs(eval_mindif) > 5e-5 and abs(eval_mindif) < 1e-4:
           #     self.delt = 0.0005
            #    self.time_counter = 2
         #   if abs(eval_mindif) > 1e-4:
          #      self.time_counter = 2
    ...
    #        print("dt for CIs", m1_list)
      ...
      #      print("evals")
            ...
            if np.allclose(eve_no_diag, zero,rtol=0, atol= 1e-5) == False:
                ...
                self.print_data( self.current_time )
                quit()
  ...
  #      print(np.allclose(projected_MF, self.tot_system.mf1RDM,rtol=0, atol=10e-10))
   ...
   #     print("Diagonalizing env of mf RDM with R after making new rdm")
    #    self.tot_system.eigvec_frag_MF_check()
        ...
        ddt_glob1RDM, iddt_NOevec, iddt_mf1RDM, G_site = mf1rdm_timedep_mod.get_ddt_mf1rdm_serial( self.dG, self.tot_system, round(self.tot_system.Nele/2) )
