...
import real_time_elec_structureGN.projected_dmet.mf1rdm_timedep_modGN as mf1rdm_timedep_mod
...
class dynamics_driver():
    ...
    def __init__( self, h_site, V_site, hamtype, tot_system, delt, Nstep, Nprint=100, integ='rk1', nproc=1, hubsite_indx=None, init_time=0.0):
       ...
       # print("evals", self.tot_system.NOevals)
#        np.save("U1Glob1rdm", self.tot_system.glob1RDM)
#        old_MF = np.copy(self.tot_system.mf1RDM)
...
#        quit()
        ...
        #Define output files
        self.file_output   = open( 'outputGN0to1dt00001CommE-6.dat', 'w' )
        self.file_corrdens = open( 'corr_densityGN0to1dt00001CommE-6.dat', 'w' )

        print("MF before rk4", self.tot_system.mf1RDM)
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            if( np.mod( step, self.Nprint ) == 0 ) and step > 3:
    ...
    def integrate( self, nproc ):
        ...
        if( self.integ == 'rk4' ):
            ...
            self.substep = 1
            l1, k1_list, m1_list, n1 = self.one_rk_step(nproc)
            #print("end of first rk dt forming")
            ...
            print("######################")
            self.substep = 2
            l2, k2_list, m2_list, n2= self.one_rk_step(nproc)
            ...
            l3, k3_list, m3_list, n3= self.one_rk_step(nproc)
            ...
            l4, k4_list, m4_list, n4 = self.one_rk_step(nproc)
           ...
           # print("NO evecs", self.tot_system.NOevecs)
            ...
            if (np.allclose(eve_no_diag, zeros, 1e-12) == False):
                quit()
            #quit()
        else:
    ...
    def one_rk_step( self, nproc ):
        ...
        #print()
        print("GlobalRDM")
        print(self.tot_system.glob1RDM)
        print()
        print("NO evecs", self.tot_system.NOevecs)
       ...
       # if self.substep == 3:
        ...
        print("formed global:", self.tot_system.glob1RDM)
        self.tot_system.glob1RDM = np.copy(propagatedGlob)
        print("propagate global:", self.tot_system.glob1RDM)
        print("##############################################")
        ...
        if (np.allclose(np.sort(self.tot_system.NOevals)[::-1], self.tot_system.NOevals, 1e-12) == False):
            print("########################")
            print("REORDERING EVALS AND NOs")
            print("########################")

        #    print("self.tot_system.NOevals before reshufle")
#            print(self.tot_system.NOevals)
         #   print("Evecs before reshufle")
 #           print(self.tot_system.NOevecs)


            idx = self.tot_system.NOevals.argsort() [::-1]
            self.tot_system.NOevals = np.copy(self.tot_system.NOevals[idx])
            self.tot_system.NOevecs = np.copy(self.tot_system.NOevecs[:, idx])
          #  print("self.tot_system.NOevals after reshufle")
  #          print(self.tot_system.NOevals)
           # print("Evecs after reshufle")
   #         print(self.tot_system.NOevecs)

            #quit()

        self.tot_system.get_new_mf1RDM(round(self.tot_system.Nele/2))
        print("mean field rdm", self.tot_system.mf1RDM)

        # print("Diagonalizing env of mf RDM with R before making new rdm")
        #print("############### MFCHECK #####################")
      ...
      #  self.tot_system.get_new_mf1RDM(round(self.tot_system.Nele/2))
      #  print("new MF", self.tot_system.mf1RDM)
     #   print("Diagonalizing env of mf RDM with R after making new rdm")
       # self.tot_system.eigvec_frag_MF_check()
        ...
        ddt_glob1RDM, ddt_NOevec, ddt_mf1RDM = mf1rdm_timedep_mod.get_ddt_mf1rdm_serial( self.tot_system, round(self.tot_system.Nele/2) )
...
#        change_mf1RDM = ddt_mf1RDM * self.delt
        ...
        if( nproc == 1 ):
            ...
            frag_pool.close()
        return change_NOevecs, change_rotmat_list, change_CIcoeffs_list, change_glob1RDM
    ...
    def print_data( self, current_time ):
        ...
        #diag_frag1 = self.tot_system.eigvec_frag_MF_check()
        #print(diag_frag1)
       ...
       # self.tot_system.get_nat_orbs()
        ...
        #print('ddt_mf1rdm call in print routine')
        ...
        #output[10+3*Nsites-Nimp+2*Nimp] = np.copy(diag[0][5])
        #output[11+3*Nsites-Nimp+2*Nimp] = np.copy(diag[1][5])
        #output[12+3*Nsites-Nimp+2*Nimp] = np.copy(diag[3][5])
        ##output[13+3*Nsites-Nimp+2*Nimp] = np.copy(diag_frag1[3][5])
        #output[14+3*Nsites-Nimp+2*Nimp] = np.copy(diag_frag1[4][5])
...
def applyham_wrapper( frag, delt ):
    ...
    print("Xmat_sml", Xmat_sml)
    print("frag.Nimp", frag.Nimp)
    print("frag.Ecore", frag.Ecore)
    print("delt", delt)
