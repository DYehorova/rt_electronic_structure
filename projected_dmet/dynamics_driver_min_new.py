   ...
   #     self.tot_system.get_nat_orbs()
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            if self.min_step is not None and (eval_min_dif <= self.dG) and step <= self.min_step:
                #self.delt = 0.0001 
    #        if self.min_step is not None and (step <= self.min_step):
                print("before min")
                self.minimization()
            else:
                #self.NOeval_dif = self.min_dif(self.tot_system.NOevals)
                #if self.NOeval_dif <= 2*self.dG:
                #    self.delt = 0.0001
                #else:
                #    self.delt = 0.001
...
#            frag_pool.join()
    ...
    def minimization(self ):
        ...
        if( self.integ == 'rk4' ):
            ...
            eval_min_dif_top = self.min_dif(self.tot_system.NOevals[:half_Nele]) 
            eval_min_dif_bottom = self.min_dif(self.tot_system.NOevals[half_Nele:])

            substep = 1
            G_bad_real, ddt_glob1RDM = self.initialize_G(half_Nele, eval_min_dif_top, eval_min_dif_bottom)
            self.itrs = 0
            if substep == 1:
                scipy.optimize.leastsq(self.cost_function_G_substep, G_bad_real, args=(half_Nele, ddt_glob1RDM, init_NOevecs, init_glob1RDM, init_mf1RDM, init_CIcoeffs_list, init_rotmat_list, substep, eval_min_dif_top, eval_min_dif_bottom))
                #least_squares(self.cost_function_G_substep, G_bad_real, verbose=1, args=(half_Nele, ddt_glob1RDM, init_NOevecs, init_glob1RDM, init_mf1RDM, init_CIcoeffs_list, init_rotmat_list, substep, eval_min_dif_top, eval_min_dif_bottom) ) 
 ...
 #####################################################################         
    def initialize_G(self, half_Nele, eval_min_dif_top, eval_min_dif_bottom):   
            ...
            if eval_min_dif_top <= self.dG and eval_min_dif_bottom <= self.dG:
            ...
            elif eval_min_dif_top <= self.dG and eval_min_dif_bottom > self.dG:  
                G_bad = self.G_site[:half_Nele, :half_Nele].flatten()
            
            elif eval_min_dif_top > self.dG and eval_min_dif_bottom <= self.dG:
                G_bad = self.G_site[half_Nele:, half_Nele:].flatten()
            else:
                print("quitting because of unknown eval degeneracy points")                  
                quit()
 ...
 #####################################################################
    ...
    def cost_function_G_substep( self, G_bad_real, half_Nele, ddt_glob1RDM, init_NOevecs, init_glob1RDM, init_mf1RDM, init_CIcoeffs_list, init_rotmat_list, substep, eval_min_dif_top, eval_min_dif_bottom):
            ...
            if eval_min_dif_top <= self.dG and eval_min_dif_bottom <= self.dG:
                ...
                new_top_G = np.reshape(G_bad, (half_Nele, half_Nele))
                new_G = np.copy(self.G_site)
                new_G[:half_Nele, :half_Nele]=np.copy(new_top_G)
                            
            elif eval_min_dif_top > self.dG and eval_min_dif_bottom <= self.dG:
                new_bottom_G = np.reshape(G_bad, (half_Nele, half_Nele))
                new_G = np.copy(self.G_site)
                new_G[half_Nele:, half_Nele:]=np.copy(new_bottom_G)
    ...
    def print_data( self, current_time ):
       ...
       # self.tot_system.get_nat_orbs()
        ...
        for q in range(1, self.tot_system.Nsites-1):
            ...
            current[q] += 0.5 * 1j * self.tot_system.h_site[r, q]*\
        ...
        current_glob = 0.5 * 1j *self.tot_system.h_site[q, p]*(self.tot_system.glob1RDM[p,q] - self.tot_system.glob1RDM[q, p])
        output[14+3*self.tot_system.Nsites:14+4*self.tot_system.Nsites] = np.copy(np.real(current_glob))
        output[15+4*self.tot_system.Nsites:15+5*self.tot_system.Nsites] = np.copy(np.imag(current_glob))
