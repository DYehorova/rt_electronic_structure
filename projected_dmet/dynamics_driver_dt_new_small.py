...
import numpy as np
import real_time_elec_structureGN.projected_dmet.system_mod_dt_new as system_mod
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            if self.NOeval_dif <= 2*self.dG:
                self.delt = 0.00001
            elif 2*self.dG < self.NOeval_dif <= 6*self.dG:
                self.delt = 0.0001
            else:
                self.delt = 0.001
            sys.stdout.flush()
...
#            frag_pool.join()
    ...
    def print_data( self, current_time ):
        ...
        output    = np.zeros((15+5*self.tot_system.Nsites))
