...
import numpy as np
import real_time_elec_structureGN.projected_dmet.system_mod_test as system_mod
import real_time_elec_structureGN.projected_dmet.mf1rdm_timedep_mod_test as mf1rdm_timedep_mod
...
#        quit()
        ...
        #ERRR
        self.old_G_site_max = 1
        self.tot_system.frag_list[0].old_Xmat_max = 1 
        self.tot_system.frag_list[1].old_Xmat_max = 1
        self.tot_system.frag_list[2].old_Xmat_max = 1
        self.time_counter = 2
    #####################################################################
    ...
    def kernel( self ):
        ...
        for step in range(self.Nstep):
            ...
            if( np.mod( step, 10 ) == 0 ) and step > 1:
            ...
            eval_mindif = self.findMinDiff(self.tot_system.NOevals)
            ...
            if abs(eval_mindif) < 1e-5:
                self.delt = 0.0001
                self.time_counter = 0
            if abs(eval_mindif) > 1e-5 and abs(eval_mindif) < 1e-4:
                self.delt = 0.0005
                self.time_counter = 1
            if abs(eval_mindif) > 1e-4:
                self.time_counter = 2
                self.delt = 0.001
    ...
    def return_max_value(self, array):
        ...
        for x in range(0, len(array)):
            for y in range(0, len(array)):
                if (abs(np.real(array[x,y])) > largest):
                    largest = array[x,y]
        return largest
  ...
  #      print(np.allclose(projected_MF, self.tot_system.mf1RDM,rtol=0, atol=10e-10))
   ...
   #     print("Diagonalizing env of mf RDM with R after making new rdm")
    #    self.tot_system.eigvec_frag_MF_check()
        ...
        ddt_glob1RDM, iddt_NOevec, iddt_mf1RDM, G_site = mf1rdm_timedep_mod.get_ddt_mf1rdm_serial( self.old_G_site_max, self.dG, self.tot_system, round(self.tot_system.Nele/2) )
       # print("ddt_mf1RDM", ddt_mf1RDM)
        #Use change in mf1RDM to calculate X-matrix for each fragment
        

        ddt_mf1RDM =  -1j * iddt_mf1RDM 
        ...
        #if abs(np.real(self.old_G_site_max))>1:
 
        change_NOevecs =  -1j * iddt_NOevec * self.delt
        change_mf1RDM =  -1j * iddt_mf1RDM * self.delt
        #else:
        ...
        ddt_mf_max = self.return_max_value(change_mf1RDM)
        ddt_NO_max = self.return_max_value(change_NOevecs)
        xmat_max = self.return_max_value(self.tot_system.frag_list[0].Xmat)
        eval_mindif = self.findMinDiff(self.tot_system.NOevals) 
        ...
        output[1] = np.real(G_site_max)
        output[2] = np.imag(G_site_max)
        output[3] = np.real(G_MF_max)
        output[4] = np.imag(G_MF_max)
        output[5] = np.real(G_NO_max)
        output[6] = np.imag(G_NO_max)
        output[7] = np.real(ddt_mf_max)
        output[8] = np.imag(ddt_mf_max)
        output[9] = np.real(ddt_NO_max)
        output[10] = np.imag(ddt_NO_max)
        output[11] = np.real(xmat_max)
        output[12] = np.imag(xmat_max)
        output[13] = self.time_counter
        output[14] = eval_mindif
        ...
        self.old_G_site_max = np.copy(G_site_max) 
    #    self.tot_system.frag_list[0].old_Xmat_max = np.copy(self.return_max_value(self.tot_system.frag_list[0].Xmat))
    #    self.tot_system.frag_list[1].old_Xmat_max = np.copy(self.return_max_value(self.tot_system.frag_list[1].Xmat))
    #    self.tot_system.frag_list[2].old_Xmat_max = np.copy(self.return_max_value(self.tot_system.frag_list[2].Xmat))
    ...
    def print_data( self, current_time ):
       ...
       # self.tot_system.get_nat_orbs()
        ...
        output[10+2*Nsites-Nimp] = self.return_max_value(self.tot_system.frag_list[0].Xmat)
