...
class dynamics_driver():
    ...
    def print_data(self, current_time):
        ...
        JL = (1j * self.tot_system.h_site[0, 1] *
              (self.tot_system.glob1RDM[0, 1]
               - self.tot_system.glob1RDM[1, 0]))

        JR = (1j * self.tot_system.h_site[2, 0] *
              (self.tot_system.glob1RDM[2, 0]
               - self.tot_system.glob1RDM[0, 2]))
        ...
        for q in range(1, self.tot_system.Nsites-1):
            rfrag = (
            self.tot_system.frag_list[self.tot_system.site_to_frag_list[q+1]])
            qfrag = (
            self.tot_system.frag_list[self.tot_system.site_to_frag_list[q]])
            pfrag = (
            self.tot_system.frag_list[self.tot_system.site_to_frag_list[q-1]])
            ...
            current_rot[q] = (0.5 *1j* self.tot_system.h_site[q, p]*
                 (np.linalg.multi_dot([qfrag.rotmat[p, qindx], qfrag.corr1RDM,
                 qfrag.rotmat[q,qindx].conj().T]) -
                    (np.linalg.multi_dot([pfrag.rotmat[p, pindx],
                    pfrag.corr1RDM,
                    pfrag.rotmat[q,pindx].conj().T])).conj().T))

            current_rot[q] += (0.5 * 1j* self.tot_system.h_site[r, q]*
                 ((np.linalg.multi_dot([rfrag.rotmat[r,rindx], rfrag.corr1RDM,
                 rfrag.rotmat[q,rindx].conj().T])).conj().T -
                    (np.linalg.multi_dot([qfrag.rotmat[r,qindx],
                    qfrag.corr1RDM,
                    qfrag.rotmat[q,qindx].conj().T]))))
            current_rot[q] *=-(1/(-0.001))
            current_glob[q] = (-1/(-0.001) *1j * self.tot_system.h_site[q, p]*
            (self.tot_system.glob1RDM[p,q] - self.tot_system.glob1RDM[q, p])
        '''
        left_frag = (
        ...
        current[5] = ((1/(-0.001))*(corrdens_short[0]
                                    - self.corrdens_old[0]) / self.delt)
        current[6] = ((1/(-0.001))*(corrdens_short[1]
                                    - self.corrdens_old[1]) / self.delt)
        current[7] = ((1/(-0.001))*(corrdens_short[2]
                                    - self.corrdens_old[2]) / self.delt)
        ...
        np.savetxt(self.file_current,
                   current.reshape(1, current.shape[0]), fmt_str)
